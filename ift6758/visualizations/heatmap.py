...
def plot_coords_on_heatmap(x_s : List[int], y_s : List[int], image=heatmap):
    ...
    for x, y in zip(x_s, y_s):
        ...
        x.append(image_coords_x)
        y.append(image_coords_y)
    hm, xedges, yedges = np.histogram2d(x, y, bins=(100, 100))
    plt.imshow(heatmap, extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]], aspect='auto')
    plt.imshow(hm.T, extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]], origin='lower', cmap='hot', alpha=0.6)
    plt.show()
